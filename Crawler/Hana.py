    ...
    try:
         ...
         for i in range(len(soup.select("table[width=622]"))):
            if "종목명" in soup.select("table[width=622]")[i].text:
                ...
                if "퇴직" in soup.text:
                    file_to_save += "퇴직"
                Title= Title_re.search(soup.select("table[width=622]")[i].text).group()
                print(Title)
                q+=1
...
def save_to_csv(html,first_day, last_day, Title, url, pdfurl):
    ...
    쿠폰평가일 = None
    if len(table)>4:
        쿠폰Range = per_re.search(table[5].text).group()
        쿠폰수익률 = per_re2.search(table[5].text).group()
        쿠폰평가일 = []

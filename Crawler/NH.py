...
def dart_api(start_date, end_date):
    ...
    for y in range(int(num_of_page)):
        print(1)
        page_no = y + 1
        new_url = "http://dart.fss.or.kr/api/search.xml?auth=" + API_KEY + "&crp_cd=" + company_code + "&start_dt="+start_date+"&end_dt="+end_date+"&dsp_tp=C&fin_rpt=Y&series=asc&page_no=" + str(
            int(page_no))
...
def read_url(url,pdfurl):
    ...
    최종관찰일평가일_re2 = re.compile("만기상환평가일 :.{0,50}(\d{4,4}년\s\d*월\s\d*일)")
    최종관찰일평가일_re = re.compile("만기상환평가일 :\s{0,2}(\d*년\s\d*월\s\d*일)")
    Month_re = re.compile("매월 월수익지급평가일에 기초자산의.*?(\d{1,2}%)")
    Month_coupon_re = re.compile("총액면가액 × (\d[.]\d{1,5}%)")
    Title_re = re.compile("NH투자증권.*?[(](\w{1,3})[)].*?제(\d*)회")
    q = 0
    ...
    try:
        ...
        for i in soup.find_all("table", {"width": "411"}):
            i.decompose()
        for i in range(len(soup.select("table[width=617]")) // 2):
            a = 0
            file_to_save = ""
            file_to_save += str(soup.select("table[width=617]")[2 * i])
            ...
            file_to_save += str(최초기준가격평가일_re.findall(soup.text)[i])
            file_to_save += str(최종관찰일평가일_re.findall(soup.text)[i])

            first_day = str(최초기준가격평가일_re.findall(soup.text)[i])

            last_day = str(최종관찰일평가일_re.findall(soup.text)[i])
            if 최종관찰일평가일_re.findall(soup.text)[i] != 최종관찰일평가일_re2.findall(soup.text)[i]:
                last_day += 최종관찰일평가일_re2.findall(soup.text)[i]
            #Title = "NH투자증권(" + Title_re.findall(soup.text)[i][0] + ")" + Title_re.findall(soup.text)[i][1]
            #print(Title)
            ...
            if "평가일자" in soup.select("table")[q - 3].text:
                ...
                M += 1
                print("월수익이네")
                save_to_csv(file_to_save,  first_day, last_day, url, pdfurl, month_range=month_range,
                            Month_coupon=Month_coupon)
            else:
                q += 1
                save_to_csv(file_to_save,  first_day, last_day,url, pdfurl)
    except Exception as e:
        print("--------------------" + str(e) + "---------------------------")

def save_to_csv(html, first_day, last_day, url, pdfurl, month_range=None, Month_coupon=None):
    ...
    뽑아낸기초자산 = re.sub("\n", "",re.sub(" ", "", re.sub(" / ", ",", re.sub("지수", "", get_td(table[0], 1, "기초자산"))))).split(",")
    ...
    만기장벽 = None
    if "미만으로 하락한 기초자산이 없는 경우" in get_xy(table[1], 1, 0):
        만기장벽 = per_re.search(get_xy(table[1], 1, 0)).group()
        if not month_range:
            if per_re2.search(get_xy(table[1], 1, 1)).group() != 만기수익률:
                만기장벽 = str(만기장벽) + "(수익률 : " + per_re2.search(
                    get_xy(table[1], 1, 1)).group() + ")"


    상품구조 = ""
    if "사채" in soup.text:
        if not "없음" in soup.text:
            상품구조 += "HiFive"
            if "이하에 있는 경우" in soup.text:
                상품구조 = "UOC"
        else:
            상품구조 += "DigitalCall"
            만기수익률 = 만기수익률 +", "+ per_re2.search(get_xy(table[1], 1, 1)).group()
    else:
        상품구조 += "StepDn"
    if 상품구조 != "DigitalCall":
        if 만기장벽:
...
def get_date(table):
    ...
    for tr in trs:
        tds = tr.find_all('td')
        if len(tds[0].text) < 3:
            range_list +={tds[1].text}
            '''if len(date_re.findall(tds[1].text))>2:
                range_text= ""
                for i in date_re.findall(tds[1].text):
                    range_text += str(i)
                range_list +={range_text}
            else:
